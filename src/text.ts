...
export function registerPomodoroHandlers(client: Client) {
  client.on('messageCreate', async (message: Message) => {
    ...
    if (content.includes('開始') || content.includes('再開')) {
      const existing = await db.collection('pomodoro_sessions').doc(message.author.id).get();
      if (existing.exists) {
  ...
  setInterval(async () => {
    ...
    if (jst.getHours() === 5 && jst.getMinutes() === 0) {
      const snapshot = await db.collection('pomodoro_sessions').get();
...
export async function startNotifyTimer(client: Client, userId: string) {
...
export function stopNotifyTimer(userId: string) {
